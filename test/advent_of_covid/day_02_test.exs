defmodule AdventOfCovid.Day02Test do
  use ExUnit.Case

  alias AdventOfCovid.Day02
  alias AdventOfCovid.Day02.Policy
  ...
  test "parses input" do
    results = Day02.parse_input(@example_input)

    assert results ==
             [
               {"abcde", %Policy{letter: "a", max: 3, min: 1}},
               {"cdefg", %Policy{letter: "b", max: 3, min: 1}},
               {"ccccccccc", %Policy{letter: "c", max: 9, min: 2}}
             ]
  end

  test "validates password (1)" do
    assert Day02.password_follows_policy?("abcde", %Policy{letter: "a", max: 3, min: 1})
    assert Day02.password_follows_policy?("ccccccccc", %Policy{letter: "c", max: 9, min: 2})
    refute Day02.password_follows_policy?("cdefg", %Policy{letter: "b", max: 3, min: 1})
  end

  test "validates password (2)" do
    assert Day02.password_follows_policy2?("abcde", %Policy{letter: "a", max: 3, min: 1})
    refute Day02.password_follows_policy2?("ccccccccc", %Policy{letter: "c", max: 9, min: 2})
    refute Day02.password_follows_policy2?("cdefg", %Policy{letter: "b", max: 3, min: 1})
  end

  test "checks input (1)" do
    assert Day02.check_input(@example_input) == 2
  end

  test "exercise (1)" do
    assert Day02.check_input(@puzzle_input) == 542
  end

  test "exercise (2)" do
    assert Day02.check_input2(@puzzle_input) == 360
  end
end
